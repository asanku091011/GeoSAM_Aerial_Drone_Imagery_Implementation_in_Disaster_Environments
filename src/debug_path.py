...
# Set start and goal
startx,starty=75,75
goalx,goaly=config.MAP_WIDTH-75,config.MAP_HEIGHT-75
start = (startx, starty)
goal = (goalx, goaly)
...
while not (map_builder.is_cell_free(start[0],start[1]) and startx<goalx):
...
while not (map_builder.is_cell_free(goal[0],goal[1]) and goalx>startx):
